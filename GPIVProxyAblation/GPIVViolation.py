...
def generate_data(n_samples=100, f_type='absolute', rho=0.0, seed=None):
    """
    Generate IV data.
    rho controls the strength of violation of independence (cov(e,w)=rho).
    """
    if seed is not None:
...
class GPIV:
    ...
    def compute_posterior_mse(self, f_type='absolute', n_points=200):
        x_test = torch.linspace(0.0, 1.0, n_points, dtype=torch.float64)
    ...
    def visualize_posterior(self, f_type='absolute', n_points=200):
        mse, x_test, h_x, means = self.compute_posterior_mse(f_type, n_points)
        ...
        if self.X_train is not None:
            plt.scatter(self.X_train.numpy(), self.y_train.numpy(),
...
def run_gpiv_experiment_with_rejection(f_type='absolute', n_samples=300,
                                       ...
                                       train_sigma=True, eta=1.0, rho=0.0):
    torch.manual_seed(seed)
    np.random.seed(seed)

    x, y, z, h_true = generate_data(n_samples, f_type=f_type, rho=rho, seed=seed)
...
def run_gpiv_multiple_experiments(n_experiments=25, f_type='absolute', n_samples=300,
                                  ...
                                  n_iterations=150, lr=0.02,
                                  train_lengthscale_x=True, train_lengthscale_z=True,
                                  train_sigma=True, eta=1.0, seed_head=500, rho=0.0):
    print(f"\n{'='*60}")
    ...
    print(f"rho (violation strength): {rho}")
...
def run_single_gpiv_demo(f_type='absolute', n_samples=300, seed=91,
    ...
    history = gp.optimize_hyperparameters(n_iterations=25, lr=0.05, verbose=True)

    print(f"\nFinal NLL: {gp.negative_log_marginal_likelihood().item():.3f}")

    gp.visualize_optimization(history)

    gp.visualize_posterior(f_type=f_type)
...
if __name__ == "__main__":
    ...
    RHO = 0.05  # strength of violation of independence (cov(e,w))

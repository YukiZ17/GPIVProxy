...
def generate_data(n_samples: int = 100, seed: Optional[int] = None):
    if seed is not None:
...
class ProxyGaussianProcess:
    ...
    def predict_mean_only(self, t_new, standardized=True):
        return self.posterior_mean(t_new, standardized)
    ...
    def compute_true_structural_function(self, t_values, n_mc=10000, standardized=True):
        if standardized:
            t_values_orig = unstandardize_data(t_values, self.t_mean, self.t_std)
        else:
    ...
    def compute_arc_analysis(self, t_test_orig, h_true_orig, quantile=0.75, remove_step=10, use_bootstrap=False, bootstrap_means=None, bootstrap_variances=None):
...
def run_experiment(config: Dict):
    ...
    if use_bootstrap:
        ...
        for b in range(bootstrap_iterations):
            ...
            _, T_mean_bootstrap, T_std_bootstrap = standardize_data(T_bootstrap)
            _, Y_mean_bootstrap, Y_std_bootstrap = standardize_data(Y_bootstrap)
            ...
            gp_bootstrap.set_data(
                z=Z_bootstrap,
                u=U_bootstrap,
                t=T_bootstrap,
                y=Y_bootstrap,
                t_mean=T_mean_bootstrap,
                t_std=T_std_bootstrap,
                y_mean=Y_mean_bootstrap,
                y_std=Y_std_bootstrap
            )
...
def run_multiple_experiments(config: Dict):
    ...
    if len(all_coverages) > 0:
        ...
        for q in quantiles:
            auc_vectors_list = []
            for exp_result in all_arc_results[q]:
                auc_vectors_list.append(exp_result['auc_vector'])

            min_len = min(len(vec) for vec in auc_vectors_list)
            auc_vectors_truncated = [vec[:min_len] for vec in auc_vectors_list]
            auc_vectors_truncated = np.array(auc_vectors_truncated)
        ...
        accuracy_vectors = {}
        for q in quantiles:
            accuracy_vectors_list = []
            for exp_result in all_arc_results[q]:
                accuracy_vectors_list.append(exp_result['proportions_below_threshold'])

            min_len = min(len(vec) for vec in accuracy_vectors_list)
            accuracy_vectors_truncated = [vec[:min_len] for vec in accuracy_vectors_list]
            accuracy_vectors_truncated = np.array(accuracy_vectors_truncated)

            accuracy_mean = np.mean(accuracy_vectors_truncated, axis=0)
            accuracy_se = np.std(accuracy_vectors_truncated, axis=0) / np.sqrt(len(accuracy_vectors_truncated))
        ...
        print(f"Original scale MSE: mean = {mse_orig_mean:.4f}, SE = {mse_orig_se:.4f}")
        print(f"Standardized scale MSE: mean = {mse_std_mean:.4f}, SE = {mse_std_se:.4f}")
        ...
        plot_results(accuracy_vectors, auc_vectors, quantiles, use_bootstrap)
...
def plot_results(accuracy_vectors, auc_vectors, quantiles, use_bootstrap):
